...
Each notification is enriched with a concise AI-generated summary via the Gemini API. The summarizer includes automatic retries with exponential backoff and gracefully falls back to a short description excerpt if the API is unavailable.

### Request Rate Limiting

A sliding-window rate limiter protects both the GitHub API integration (5,000 req/hour) and the webhook endpoints (60 req/min per IP).
...
- Google Gemini API key — free tier at [Google AI Studio](https://aistudio.google.com/app/api-keys)
...
Each notification includes a concise 1-2 sentence AI summary powered by Gemini. If the API is unavailable or rate-limited, the bot gracefully falls back to a short description excerpt.
