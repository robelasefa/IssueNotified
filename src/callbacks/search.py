"""
Search command callback handler.

Supports two search modes:
  /search react               → full-text search across GitHub
  /search facebook/react      → scoped to owner, returns matching repos
"""
...
# so we encode as track|owner|repo and split on the pipe character,
# which is safe because GitHub names cannot contain "|".
_CB_PREFIX = "track|"
...
def _parse_track_callback(data: str):
    """Return (owner, repo) from a track callback payload."""
...
async def _delete_loading(context, chat_id: int) -> None:
    """Delete the 'Searching…' message if it was saved."""
...
def _build_results_message(
    ...
    for i, repo in enumerate(results[:10], 1):
        ...
        repo_url = repo.get("url", f"https://github.com/{owner}/{name}")
        lines.append(
...
async def search_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /search <query>  or  /search owner/repo."""
    if not context.args:
    ...
    try:
        ...
        logger.error(f"search_repositories raised unexpectedly: {e}")
...
# ---------------------------------------------------------------------------
# Callback handler  (inline "Track" buttons)
# ---------------------------------------------------------------------------


async def handle_search_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle ➕ Track button presses from search results."""
    ...
    if success:
        logger.info(f"User {user_id} added {owner}/{repo} via search")
        await query.edit_message_text(
