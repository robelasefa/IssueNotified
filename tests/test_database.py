...
def test_add_user(db):
    ...
    row = db._query_one("SELECT username FROM users WHERE user_id = 12345")
...
def test_delete_user(db):
    ...
    row = db._query_one("SELECT * FROM users WHERE user_id = 12345")
    assert row is None


def test_tracked_issues(db):
    repo_id = db.add_repository("owner", "repo")
    db.add_tracked_issue("issue_1", repo_id, "Title", "http://url", "open")

    issues = db.get_tracked_issues_for_repo(repo_id)
    assert "issue_1" in issues
    assert issues["issue_1"] == "open"
...
def test_cleanup_unused_repositories(db):
    ...
    row = db._query_one("SELECT id FROM repositories WHERE id = ?", (repo_id,))
