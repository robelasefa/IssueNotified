from src.notifier import matches_keywords


def test_matches_keywords_no_keywords():
    issue = {"title": "Hello World", "description": "This is a test", "tags": ["bug"]}
    assert matches_keywords(issue, "") is True
    assert matches_keywords(issue, None) is True


def test_matches_keywords_in_title():
    issue = {"title": "CRITICAL: Database down", "description": "...", "tags": []}
    assert matches_keywords(issue, "critical") is True
    assert matches_keywords(issue, "safe") is False
